...
const Dashboard = () => {
  ...
  const handleCompleteRequest = async (request) => {
    try {
      const res = await api.patch(`/skills/request/${request._id}/complete`);
      ...
      setRequests((prev) =>
        prev.map((r) =>
          r._id === request._id ? { ...r, status: "Completed" } : r
        )
      );
  ...
  const submitReview = async () => {
    try {
      await api.post("/reviews", {
        skill: selectedRequest.skill._id,
  ...
  const downloadCertificate = async (requestId) => {
    try {
      const response = await api.get(`/certificate/${requestId}`, {
        responseType: "blob",
      });
      ...
      link.href = url;
      link.setAttribute("download", "SkillBarter-Certificate.pdf");
  ...
  const filteredRequests = requests.filter((r) => {
    ...
    if (requestFilter === "month") return date.isAfter(now.startOf("month"));

    if (requestFilter === "year") return date.isAfter(now.startOf("year"));

    return true;
  });

  const receivedRequests = filteredRequests.filter(
    (r) => r.provider?._id === user._id
  );

  const sentRequests = filteredRequests.filter(
  ...
  const handleVerifyCode = async (requestId) => {
    try {
      await api.patch(`/skills/request/${requestId}/verify-code`, {
        code: verificationCodes[requestId],
      });
    ...
    <div className="flex h-[calc(100vh-80px)] w-full bg-slate-950 text-white relative overflow-hidden">
              ...
              <div>
                ...
                {filteredRequests.length > 0 ? (
                  <>
                        ...
                        >
                          <div>
                            ...
                            {r.status === "Accepted" && (
                              <p className="text-sm text-yellow-400 mt-2 font-semibold">
                                🔐 Completion Code: {r.completionCode}
                              </p>
                            )}
                          </div>
                          ...
                          <div className="flex gap-2">
                            ...
                            {r.status === "Accepted" && (
                              <button
                                onClick={() =>
                                  window.open(r.skill.meetingLink, "_blank")
                                }
                    ...
                    {/* Requests Sent */}
                    <h3 className="text-xl font-semibold text-teal-400 mt-8 mb-4">
                            ...
                            {r.requester?._id === user._id &&
                              ...
                              !r.codeVerified && (
                                <div className="flex gap-2">
                                  ...
                                  <button
                                    onClick={() => handleVerifyCode(r._id)}
      ...
      {showReviewModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-[90%] max-w-md">
